...
final class Companion: ObservableObject {
    ...
    /// Kick off with a greeting that's actually useful — what's waiting on
    /// you — rather than "how can I help".
    func openingBrief() async {
        guard turns.isEmpty else { return }
        history.append(.text("user", "[The user just opened voice mode. Call list_agents, then greet them in ONE short sentence saying what needs them right now, or that nothing does. Do not list the tabs.]"))
        await run()
    }
